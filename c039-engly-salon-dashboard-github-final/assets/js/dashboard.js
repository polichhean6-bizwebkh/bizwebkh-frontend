...
(function () {
  ...
  // Owner Activity Log - append-only operational audit log (see logActivity() below). Deliberately
  // NOT included in ALL_KEYS: a "reset demo data" action should not silently wipe an audit trail,
  // and the spec explicitly requires the log to never fabricate/lose history from the normal UI.
  const KEY_ACTIVITY_LOG = "c039_activity_log";
  ...
  // `displayName` supports multiple Owner accounts each showing their own real name in the
  // Activity Log (per client spec) instead of a generic "Owner" label; accounts without one fall
  // back to their capitalized username (currentEditorLabel()), unchanged from prior behavior.
  const DEMO_ACCOUNTS = [
    { username: "owner", password: "demo123", role: "owner", displayName: "Engly Khun" },
    { username: "owner2", password: "demo123", role: "owner", displayName: "Owner Account 2" },
  ...
  function generateInitialRecords(transactions) {
    ...
    transactions.filter((tx) => tx.status === "Completed").forEach((tx) => {
      ...
      // Sales Incentive record(s) (package/product sale closed by Sales Staff) - separate from
      // Service Incentive. One record per Sales Staff attached to the sale (salesStaffList); falls
      // back to the legacy single-name fields for older seed transactions without a list.
      const saleServiceLabel = tx.txType === "vip_topup" ? "VIP Top-Up" : "VIP Package Purchase";
      const salesEntries = Array.isArray(tx.salesStaffList) && tx.salesStaffList.length
        ? tx.salesStaffList
        : (tx.salesStaff && tx.salesIncentiveAmount ? [{ staff: tx.salesStaff, incentiveRate: 0, incentiveAmount: tx.salesIncentiveAmount }] : []);
      salesEntries.forEach((entry) => {
        if (!entry.staff || !entry.incentiveAmount) return;
        records.push({
          id: "INC-" + String(n++).padStart(3, "0"),
          transactionId: tx.id,
          rowIndex: -1,
          ...
          customer: tx.customerName,
          phone: tx.phone,
          service: saleServiceLabel,
          price: tx.grandTotal,
          qty: 1,
          staff: entry.staff,
          incentiveType: "sale",
          incentiveRate: entry.incentiveRate || 0,
  ...
  const I18N = {
    km: {
      ...
      "nav.activityLog": "កំណត់ហេតុសកម្មភាព",
      ...
      "th.status": "ស្ថានភាព",
      "th.actions": "សកម្មភាព",
      "th.user": "អ្នកប្រើប្រាស់",
      "th.role": "តួនាទី",
      "th.module": "ផ្នែក",
      "th.action": "សកម្មភាព",
      "th.details": "ព័ត៌មានលម្អិត",
      "th.dateTime": "កាលបរិច្ឆេទ / ម៉ោង",

      "activityLog.title": "កំណត់ហេតុសកម្មភាព",
      "activityLog.searchPlaceholder": "ស្វែងរកអតិថិជន បុគ្គលិក ឬប្រតិបត្តិការ",
      "activityLog.noResults": "រកមិនឃើញសកម្មភាព",
      "activityLog.module.customers": "អតិថិជន",
      "activityLog.module.transactions": "ប្រតិបត្តិការ",
      "activityLog.module.vip": "VIP",
      "activityLog.module.staff": "បុគ្គលិក",
      "activityLog.module.incentive": "កម្រៃជើងសារ",
      "activityLog.module.services": "សេវាកម្ម",
      "activityLog.module.settings": "ការកំណត់",
      "activityLog.action.customerCreated": "បង្កើតអតិថិជនថ្មី",
      "activityLog.action.customerEdited": "កែប្រែអតិថិជន",
      "activityLog.action.customerTypeChanged": "ផ្លាស់ប្តូរប្រភេទអតិថិជន",
      "activityLog.action.transactionCreated": "បង្កើតប្រតិបត្តិការ",
      "activityLog.action.transactionCompleted": "បញ្ចប់ប្រតិបត្តិការ",
      "activityLog.action.transactionEdited": "កែប្រែប្រតិបត្តិការ",
      "activityLog.action.transactionCancelled": "លុបចោលប្រតិបត្តិការ",
      "activityLog.action.vipPackagePurchase": "ទិញកញ្ចប់ VIP",
      "activityLog.action.vipPackageTopup": "បញ្ចូលទឹកប្រាក់ VIP",
      "activityLog.action.vipServiceRedemption": "ប្រើប្រាស់សមតុល្យ VIP",
      "activityLog.action.staffCreated": "បន្ថែមបុគ្គលិកថ្មី",
      "activityLog.action.staffEdited": "កែប្រែបុគ្គលិក",
      "activityLog.action.staffStatusChanged": "ផ្លាស់ប្តូរស្ថានភាពបុគ្គលិក",
      "activityLog.action.incentiveConfigChanged": "កែប្រែការកំណត់កម្រៃជើងសារ",
      "activityLog.action.serviceAdded": "បន្ថែមសេវាកម្មថ្មី",
      "activityLog.action.servicePriceUpdated": "កែប្រែតម្លៃសេវាកម្ម",
      "activityLog.action.serviceRemoved": "លុបសេវាកម្ម",
      ...
      "login.owner2Hint": "ម្ចាស់ (គណនីទី ២)",
      ...
      "staff.selectStaffPlus": "+ ជ្រើសរើសបុគ្គលិក",
      ...
      "export.modalTitle": "នាំចេញទិន្នន័យអតិថិជន",
      "export.modalHint": "ជ្រើសរើសព័ត៌មានដែលអ្នកចង់នាំចេញទៅ Excel",
      "export.needOneField": "សូមជ្រើសរើសយ៉ាងហោចណាស់មួយវាល",
      "export.field.name": "ឈ្មោះអតិថិជន",
      "export.field.phone": "លេខទូរស័ព្ទ",
      "export.field.type": "ប្រភេទអតិថិជន",
      "export.field.source": "ប្រភពអតិថិជន",
      "export.field.firstVisit": "កាលបរិច្ឆេទមកលេងលើកដំបូង",
      "export.field.lastVisit": "កាលបរិច្ឆេទមកលេងចុងក្រោយ",
      "export.field.totalVisits": "ចំនួនដងមកលេងសរុប",
      "export.field.revenue": "ការចំណាយសរុប / ចំណូលរួមចំណែក",
      "export.field.mostUsed": "សេវាកម្មប្រើប្រាស់ច្រើនបំផុត",
      "export.field.vipBalance": "សមតុល្យ VIP",
      "export.field.vipPackageInfo": "ព័ត៌មានទិញ/បញ្ចូលបន្ថែមកញ្ចប់ VIP",
    ...
    en: {
      ...
      "activityLog.searchPlaceholder": "Search customer, staff, or transaction",
      "activityLog.noResults": "No activity found",
      "activityLog.module.customers": "Customers",
      "activityLog.module.transactions": "Transactions",
      "activityLog.module.vip": "VIP",
      "activityLog.module.staff": "Staff",
      "activityLog.module.incentive": "Incentive",
      "activityLog.module.services": "Services",
      "activityLog.module.settings": "Settings",
      "activityLog.action.customerCreated": "Customer Created",
      "activityLog.action.customerEdited": "Customer Edited",
      "activityLog.action.customerTypeChanged": "Customer Type Changed",
      "activityLog.action.transactionCreated": "Transaction Created",
      "activityLog.action.transactionCompleted": "Transaction Completed",
      "activityLog.action.transactionEdited": "Transaction Edited",
      "activityLog.action.transactionCancelled": "Transaction Cancelled",
      "activityLog.action.vipPackagePurchase": "VIP Package Purchase",
      "activityLog.action.vipPackageTopup": "VIP Package Top-Up",
      "activityLog.action.vipServiceRedemption": "VIP Balance Redemption",
      "activityLog.action.staffCreated": "Staff Added",
      "activityLog.action.staffEdited": "Staff Edited",
      "activityLog.action.staffStatusChanged": "Staff Status Changed",
      "activityLog.action.incentiveConfigChanged": "Incentive Configuration Updated",
      "activityLog.action.serviceAdded": "Service Added",
      "activityLog.action.servicePriceUpdated": "Service Price Updated",
      "activityLog.action.serviceRemoved": "Service Removed",
      ...
      "login.owner2Hint": "Owner (2nd account)",
      ...
      "staff.selectStaffPlus": "+ Select Staff",
      ...
      "export.modalTitle": "Export Customer Data",
      "export.modalHint": "Choose which customer information to include in the Excel file.",
      "export.needOneField": "Please select at least one field to export.",
      "export.field.name": "Customer Name",
      "export.field.phone": "Phone Number",
      "export.field.type": "Customer Type",
      "export.field.source": "Customer Source",
      "export.field.firstVisit": "First Visit Date",
      "export.field.lastVisit": "Last Visit Date",
      "export.field.totalVisits": "Total Visits",
      "export.field.revenue": "Total Spending / Revenue Contribution",
      "export.field.mostUsed": "Most-used Service",
      "export.field.vipBalance": "VIP Balance",
      "export.field.vipPackageInfo": "VIP Package Purchase / Top-Up Information",
  ...
  const PAGE_TITLE_KEYS = {
    ...
    reports: "nav.reports", "activity-log": "nav.activityLog", settings: "nav.settings"
  };
  ...
  function goToPage(page) {
    // Owner-only page: defensive guard, in case a Cashier session somehow lands here directly
    // (the nav item itself is already hidden for Cashier via applyRole()).
    if (page === "activity-log" && getSettings().role !== "owner") page = "overview";
  ...
  function applyRole() {
    ...
    // Activity Log is Owner-only (per spec: mainly for Owners to see actions by other authorized
    // users). This is the first nav item in the app that is actually hidden by role - if the
    // Cashier is currently on that page when role/session changes, bounce them to Overview.
    const navActivityLog = document.getElementById("navActivityLog");
    if (navActivityLog) {
      navActivityLog.classList.toggle("hidden", role !== "owner");
      if (role !== "owner" && navActivityLog.classList.contains("active")) goToPage("overview");
    }
  }
  ...
  function completeActiveDeal(validRows, totals) {
    ...
    const tx = {
      ...
      status: "Completed",
      txType: "service",
      // Staff attribution for a regular service transaction lives entirely on each service row's
      // own Staff selector (see services below) - there is no standalone "Sales Staff" for the
      // whole transaction, and no sales-type incentive is generated from a service transaction.
    ...
    logActivity({
      module: "transactions", action: "transaction_created",
      entityType: "transaction", entityId: tx.id,
      details: `${t("th.customer")}: ${tx.customerName} / ${t("th.total")}: ${fmt(tx.grandTotal)} / ${t("th.payment")}: ${tx.payment}`
    });
    if (vipDeduction > 0) {
      logActivity({
        module: "vip", action: "vip_service_redemption",
        entityType: "transaction", entityId: tx.id,
        details: `${t("th.customer")}: ${tx.customerName} / ${t("vip.used")}: ${fmt(vipDeduction)}`
      });
    }
  ...
  function generateIncentiveRecordsForTransaction(tx) {
    ...
    // Sales Incentive - separate from Service Incentive. One record PER Sales Staff who helped
    // close the sale (multiple staff can be attached to a single VIP package/top-up); each keeps
    // their own independently-configured incentive amount. Falls back to the legacy single-name
    // salesStaff/salesIncentiveAmount fields for older transactions that predate salesStaffList.
    const saleServiceLabel = tx.txType === "vip_topup" ? "VIP Top-Up" : (tx.txType === "vip_purchase" ? "VIP Package Purchase" : "Sale");
    const salesEntries = Array.isArray(tx.salesStaffList) && tx.salesStaffList.length
      ? tx.salesStaffList
      : (tx.salesStaff && tx.salesIncentiveAmount ? [{ staff: tx.salesStaff, incentiveRate: 0, incentiveAmount: tx.salesIncentiveAmount }] : []);
    salesEntries.forEach((entry) => {
      ...
      records.push({
        ...
        date: tx.date, customer: tx.customerName, phone: tx.phone,
        service: saleServiceLabel,
        price: tx.grandTotal, qty: 1, staff: entry.staff,
        incentiveType: "sale", incentiveRate: entry.incentiveRate || 0, incentiveAmount: entry.incentiveAmount,
        incentiveCategory: "sales",
        status: "Confirmed"
      });
    });
  ...
  /* ============================================================
     ACTIVITY LOG PAGE (Owner-only)
     Simple read-only table over KEY_ACTIVITY_LOG - no edit/delete UI anywhere here by design
     (append-only, see logActivity()). Reuses the same Today/Last7/Last30/Custom Range filter
     pattern as Transactions/Reports for consistency.
     ============================================================ */
  const ACTIVITY_ACTION_KEYS = {
    customer_created: "activityLog.action.customerCreated",
    customer_edited: "activityLog.action.customerEdited",
    customer_type_changed: "activityLog.action.customerTypeChanged",
    transaction_created: "activityLog.action.transactionCreated",
    transaction_completed: "activityLog.action.transactionCompleted",
    transaction_edited: "activityLog.action.transactionEdited",
    transaction_cancelled: "activityLog.action.transactionCancelled",
    vip_package_purchase: "activityLog.action.vipPackagePurchase",
    vip_package_topup: "activityLog.action.vipPackageTopup",
    vip_service_redemption: "activityLog.action.vipServiceRedemption",
    staff_created: "activityLog.action.staffCreated",
    staff_edited: "activityLog.action.staffEdited",
    staff_status_changed: "activityLog.action.staffStatusChanged",
    incentive_configuration_changed: "activityLog.action.incentiveConfigChanged",
    service_added: "activityLog.action.serviceAdded",
    service_price_updated: "activityLog.action.servicePriceUpdated",
    service_removed: "activityLog.action.serviceRemoved"
  };
  const ACTIVITY_MODULE_KEYS = {
    customers: "activityLog.module.customers",
    transactions: "activityLog.module.transactions",
    vip: "activityLog.module.vip",
    staff: "activityLog.module.staff",
    incentive: "activityLog.module.incentive",
    services: "activityLog.module.services",
    settings: "activityLog.module.settings"
  };
  function activityActionLabel(action) { const k = ACTIVITY_ACTION_KEYS[action]; return k ? t(k) : action; }
  function activityModuleLabel(mod) { const k = ACTIVITY_MODULE_KEYS[mod]; return k ? t(k) : mod; }

  function getActLogDateRange() {
    const range = document.getElementById("filterActLogDateRange").value;
    const fromEl = document.getElementById("filterActLogDateFrom");
    const toEl = document.getElementById("filterActLogDateTo");
    fromEl.style.display = range === "custom" ? "" : "none";
    toEl.style.display = range === "custom" ? "" : "none";
    const today = daysAgo(0);
    ...
    if (range === "custom") {
      ...
      if (from && from > today) { from = ""; fromEl.value = ""; showToast(t("filters.futureDateBlocked"), "error"); }
      if (to && to > today) { to = ""; toEl.value = ""; showToast(t("filters.futureDateBlocked"), "error"); }
      if (from && to && from > to) { showToast(t("filters.startAfterEnd"), "error"); return { from: "", to: "", invalid: true }; }
      return { from, to };
    }
    return { from: "", to: "" };
  }

  function populateActLogFilterOptions() {
    const log = load(KEY_ACTIVITY_LOG, []);
    const users = [...new Set(log.map((r) => r.userName))].sort();
    const modules = [...new Set(log.map((r) => r.module))];
    const actions = [...new Set(log.map((r) => r.action))];
    document.getElementById("filterActLogUser").innerHTML = `<option value="">${t("filters.all")} - ${t("th.user")}</option>` +
      users.map((u) => `<option value="${u}">${u}</option>`).join("");
    document.getElementById("filterActLogModule").innerHTML = `<option value="">${t("filters.all")} - ${t("th.module")}</option>` +
      modules.map((m) => `<option value="${m}">${activityModuleLabel(m)}</option>`).join("");
    document.getElementById("filterActLogAction").innerHTML = `<option value="">${t("filters.all")} - ${t("th.action")}</option>` +
  ...
  function renderActivityLogTable(list) {
    ...
    table.querySelector("thead").innerHTML = `<tr>
      <th>${t("th.dateTime")}</th><th>${t("th.user")}</th><th>${t("th.role")}</th>
      <th>${t("th.module")}</th><th>${t("th.action")}</th><th>${t("th.details")}</th>
    </tr>`;
    if (!list.length) {
      table.querySelector("tbody").innerHTML = `<tr><td colspan="6" style="text-align:center;color:#B7ABA0;padding:24px;">${t("activityLog.noResults")}</td></tr>`;
      return;
    }
    table.querySelector("tbody").innerHTML = list.map((r) => `
      <tr>
        <td>${formatDisplayDate(r.date)} · ${r.time}</td>
        <td>${r.userName}</td>
        <td>${r.role === "owner" ? t("settings.owner") : t("settings.cashier")}</td>
        <td>${activityModuleLabel(r.module)}</td>
        <td>${activityActionLabel(r.action)}</td>
        <td>${r.details || "—"}</td>
      </tr>
    `).join("");
  }
  ...
  function applyActLogFilters() {
    let list = load(KEY_ACTIVITY_LOG, []);
    const searchEl = document.getElementById("actLogSearch");
    const search = searchEl ? searchEl.value.trim().toLowerCase() : "";
    const { from, to, invalid } = getActLogDateRange();
    const user = document.getElementById("filterActLogUser").value;
    const mod = document.getElementById("filterActLogModule").value;
    const action = document.getElementById("filterActLogAction").value;

    if (search) {
      list = list.filter((r) => (r.details || "").toLowerCase().includes(search) || (r.entityId || "").toLowerCase().includes(search));
    }
    if (invalid) {
      list = [];
    } else if (from && to) {
    ...
    if (user) list = list.filter((r) => r.userName === user);
    if (mod) list = list.filter((r) => r.module === mod);
    if (action) list = list.filter((r) => r.action === action);

    // Newest first, by default
    list.sort((a, b) => (b.date + b.time + b.id).localeCompare(a.date + a.time + a.id));
    renderActivityLogTable(list);
  }

  function renderActivityLogPage() {
    if (getSettings().role !== "owner") return; // Owner-only page
    populateActLogFilterOptions();
    const fromEl = document.getElementById("filterActLogDateFrom");
    const toEl = document.getElementById("filterActLogDateTo");
    const today = daysAgo(0);
    if (fromEl) fromEl.max = today;
  ...
  function setupActLogFilters() {
    const searchEl = document.getElementById("actLogSearch");
    if (searchEl) searchEl.addEventListener("input", applyActLogFilters);
    const today = daysAgo(0);
    const fromEl = document.getElementById("filterActLogDateFrom");
    const toEl = document.getElementById("filterActLogDateTo");
    if (fromEl) fromEl.max = today;
    if (toEl) toEl.max = today;
    ["filterActLogDateRange", "filterActLogDateFrom", "filterActLogDateTo", "filterActLogUser", "filterActLogModule", "filterActLogAction"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.addEventListener("change", applyActLogFilters);
    });
    const clearBtn = document.getElementById("clearActLogFilters");
    if (clearBtn) clearBtn.addEventListener("click", () => {
      if (searchEl) searchEl.value = "";
      document.getElementById("filterActLogDateRange").value = "all";
      ["filterActLogDateFrom", "filterActLogDateTo", "filterActLogUser", "filterActLogModule", "filterActLogAction"].forEach((id) => { document.getElementById(id).value = ""; });
      applyActLogFilters();
    });
  }
  ...
  function completeTransaction(id) {
    ...
    generateIncentiveRecordsForTransaction(tx);
    logActivity({
      module: "transactions", action: "transaction_completed",
      entityType: "transaction", entityId: tx.id,
      details: `${t("th.customer")}: ${tx.customerName} / ${t("th.total")}: ${fmt(tx.grandTotal)}`
    });
  ...
  function cancelTransaction(id) {
    ...
    save(KEY_TRANSACTIONS, txs);
    logActivity({
      module: "transactions", action: "transaction_cancelled",
      entityType: "transaction", entityId: tx.id,
      details: `${t("th.customer")}: ${tx.customerName}`
    });
  ...
  function currentEditorLabel() {
    ...
    // Prefer the account's real display name (supports multiple Owner accounts each showing their
    // own name in the audit trail / Activity Log) - falls back to the capitalized username for
    // accounts that don't have one configured, unchanged from the original behavior.
    const name = (session && session.displayName) ? session.displayName : (username.charAt(0).toUpperCase() + username.slice(1));
  ...
  /* ============================================================
     OWNER ACTIVITY LOG
     Simple, append-only operational audit trail: "who did what, when". Not a compliance system -
     only meaningful actions are logged (see call sites below), never every click/navigation, and
     it never creates/duplicates any financial data itself - it only records that an action already
     performed elsewhere (customer/transaction/staff/service/incentive) took place.
     ============================================================ */
  function logActivity(entry) {
    try {
      const editor = currentEditorLabel();
      const now = new Date();
      const list = load(KEY_ACTIVITY_LOG, []);
      const nextNum = list.reduce((max, r) => {
        const m = /ACT-(\d+)/.exec(r.id || "");
        return m ? Math.max(max, parseInt(m[1], 10)) : max;
      }, 0) + 1;
      list.push({
        id: "ACT-" + String(nextNum).padStart(5, "0"),
        userId: getSession() ? getSession().username : editor.name,
      ...
      save(KEY_ACTIVITY_LOG, list);
    } catch (e) { /* never let logging break the underlying action */ }
  }
  ...
  function saveEditedTransaction(tx) {
    ...
    logActivity({
      module: "transactions", action: "transaction_edited",
      entityType: "transaction", entityId: tx.id,
      details: `${t("th.customer")}: ${tx.customerName} / ${changes.map((c) => c.field).join(", ")}`
    });
  ...
  function openEditCustomerModal(c) {
    ...
    document.getElementById("editCustSaveBtn").addEventListener("click", () => {
      ...
      logActivity({
        module: "customers", action: "customer_edited",
        entityType: "customer", entityId: c.id,
        details: `${name} (${phone})`
      });
      if (newType !== current.type) {
        logActivity({
          module: "customers", action: "customer_type_changed",
          entityType: "customer", entityId: c.id,
          details: `${name} / ${t("th.customerType")}: ${current.type === "vip" ? t("customerType.vip") : t("customerType.normal")} → ${newType === "vip" ? t("customerType.vip") : t("customerType.normal")}`
        });
      }
  ...
  // Multiple Sales Staff can be linked to one VIP package purchase/top-up (client requirement - a
  // package sale is often closed with help from more than one staff member). Rendered as removable
  // chips + a "+ Select Staff" dropdown to add more, per the client's requested UI. Deliberately
  // shows ONLY staff names here - no incentive rate/amount is ever displayed in this picker;
  // incentive calculation happens silently in the background from Owner's Staff settings.
  // `idPrefix + "Wrap"` is the container element that must already exist in the modal markup.
  function renderStaffChipPicker(idPrefix, selectedNames) {
    const wrap = document.getElementById(idPrefix + "Wrap");
    if (!wrap) return;
    const staff = load(KEY_STAFF, SEED_STAFF).filter((s) => s.active && (s.type === "sales" || s.type === "both"));
    const selected = (selectedNames || []).filter((n) => staff.some((s) => s.name === n));
    const available = staff.filter((s) => !selected.includes(s.name));
    wrap.innerHTML = `
      <div class="staff-chip-list">
        ${selected.map((name) => `
          <span class="staff-chip" data-name="${name}">${name}
            <button type="button" class="staff-chip-remove" data-name="${name}" aria-label="${t("modal.cancel")}">&times;</button>
          </span>
        `).join("")}
      </div>
      <select class="staff-chip-add" id="${idPrefix}AddSelect">
        <option value="">${t("staff.selectStaffPlus")}</option>
        ${available.map((s) => `<option value="${s.name}">${s.name}</option>`).join("")}
      </select>
    `;
    wrap.querySelectorAll(".staff-chip-remove").forEach((btn) => {
      btn.addEventListener("click", () => {
        renderStaffChipPicker(idPrefix, selected.filter((n) => n !== btn.dataset.name));
      });
    });
    const addSelect = document.getElementById(idPrefix + "AddSelect");
    if (addSelect) {
      addSelect.addEventListener("change", (e) => {
        const val = e.target.value;
        if (val && !selected.includes(val)) renderStaffChipPicker(idPrefix, [...selected, val]);
      });
    }
  }

  function getChipSelectedNames(idPrefix) {
    const wrap = document.getElementById(idPrefix + "Wrap");
    if (!wrap) return [];
    return [...wrap.querySelectorAll(".staff-chip")].map((chip) => chip.dataset.name);
  }

  // Computes each selected staff member's OWN configured Sales Incentive independently (per Owner's
  // Staff Management settings) - this is not a split/duplication of a single amount, each staff who
  // helped close the sale is credited according to their own configured rate. Returns
  // { names, list: [{staff, incentiveRate, incentiveAmount}], totalIncentiveAmount }.
  function computeMultiSalesIncentive(staffNames, saleAmount) {
    const list = (staffNames || []).filter(Boolean).map((name) => {
      const inc = computeSalesIncentive(name, saleAmount);
      return { staff: name, incentiveRate: inc.rate, incentiveAmount: inc.amount };
    });
    return {
      names: list.map((l) => l.staff),
      list,
      totalIncentiveAmount: +list.reduce((s, l) => s + (l.incentiveAmount || 0), 0).toFixed(2)
    };
  }
  ...
  function openAddCustomerModal() {
    ...
    document.getElementById("modalBody").innerHTML = `
      ...
      <div id="newCustVipFields" style="display:none;">
        ...
        <div class="form-field"><label>${t("deal.salesStaff")}</label><div class="staff-chip-select" id="newCustVipSalesStaffWrap"></div></div>
      </div>
    ...
    renderStaffChipPicker("newCustVipSalesStaff", []);
    ...
    document.getElementById("saveNewCustBtn").addEventListener("click", () => {
      ...
      if (type === "vip") {
        ...
        const salesStaffNames = getChipSelectedNames("newCustVipSalesStaff");
        ...
        if (amount > 0) {
          const inc = computeMultiSalesIncentive(salesStaffNames, amount);
          cust.vipBalance = amount;
          cust.packageHistory.push({
            id: "PKG-" + Date.now(), amount, paymentMethod: payment,
            salesStaff: inc.names.join(", "), salesStaffList: inc.list, salesIncentiveAmount: inc.totalIncentiveAmount,
            date, type: "purchase"
          });
          recordVipPackageTransaction(cust, amount, "vip_purchase", payment, inc, date);
        }
      }
      customers.push(cust);
      save(KEY_CUSTOMERS, customers);
      logActivity({
        module: "customers", action: "customer_created",
        entityType: "customer", entityId: cust.id,
        details: `${cust.name} (${cust.phone}) / ${t("th.customerType")}: ${cust.type === "vip" ? t("customerType.vip") : t("customerType.normal")}`
      });
      closeModal();
  ...
  // `inc` is the result of computeMultiSalesIncentive(): { names, list, totalIncentiveAmount }.
  // Each entry in `inc.list` keeps its own independently-configured incentive amount - stored on
  // the transaction as salesStaffList so Reports > Staff & Incentive can attribute the correct
  // incentive to each individual staff member who helped close the sale.
  function recordVipPackageTransaction(cust, amount, txType, payment, inc, date) {
    const transactions = load(KEY_TRANSACTIONS, []);
    const nextNum = transactions.reduce((max, tx) => {
      const m = /TX-(\d+)/.exec(tx.id || "");
      return m ? Math.max(max, parseInt(m[1], 10)) : max;
    ...
    const tx = {
      ...
      salesStaff: inc.names.join(", "), salesStaffList: inc.list, salesIncentiveAmount: inc.totalIncentiveAmount, vipDeduction: 0,
    ...
    save(KEY_TRANSACTIONS, transactions);
    generateIncentiveRecordsForTransaction(tx);
    // One activity entry per VIP package purchase/top-up event - matches the one-revenue-event-per-
    // sale rule (see revenueOf()/generateIncentiveRecordsForTransaction) so the log never implies a
    // second sale happened just because multiple Sales Staff were attached to this one transaction.
    logActivity({
      module: "vip", action: txType === "vip_topup" ? "vip_package_topup" : "vip_package_purchase",
      entityType: "transaction", entityId: tx.id,
      details: `${t("th.customer")}: ${cust.name} / ${txType === "vip_topup" ? t("vip.topupLabel") : t("vip.purchaseLabel")}: ${fmt(amount)}` +
        (inc.names.length ? ` / ${t("deal.salesStaff")}: ${inc.names.join(", ")}` : "")
    });
    return tx;
  }
  ...
  function openVipSellTopUpModal() {
    ...
    document.getElementById("modalBody").innerHTML = `
      ...
      <div class="form-field"><label>${t("deal.salesStaff")}</label><div class="staff-chip-select" id="vipTxSalesStaffWrap"></div></div>
      <div class="modal-actions"><button class="btn-primary" id="saveVipTxBtn">${t("modal.save")}</button></div>
    `;
    openModal();
    wireVipAmountPicker("vipTxAmount");
    renderStaffChipPicker("vipTxSalesStaff", []);
    ...
    document.getElementById("saveVipTxBtn").addEventListener("click", () => {
      ...
      const salesStaffNames = getChipSelectedNames("vipTxSalesStaff");
      const customersNow = load(KEY_CUSTOMERS, []);
      ...
      // Each package/top-up keeps its own staff attribution - previous entries in packageHistory
      // are never modified or overwritten by this push.
      cust.packageHistory.push({
        id: "PKG-" + Date.now(), amount, paymentMethod: payment,
        salesStaff: inc.names.join(", "), salesStaffList: inc.list, salesIncentiveAmount: inc.totalIncentiveAmount,
        date: daysAgo(0), type: isFirstPurchase ? "purchase" : "topup"
      });
      save(KEY_CUSTOMERS, customersNow);
      recordVipPackageTransaction(cust, amount, txType, payment, inc, daysAgo(0));
      closeModal();
      showToast(t("toast.saved"), "success");
      renderCustomersPage();
    });
    ...
    if (isOwner) {
      ...
      table.querySelectorAll('[data-action="toggle-staff"]').forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          save(KEY_STAFF, list);
          logActivity({
            module: "staff", action: "staff_status_changed",
            entityType: "staff", entityId: s.id,
            details: `${s.name} → ${s.active ? t("staff.active") : t("staff.inactive")}`
          });
          renderStaffPage();
        });
      });
  ...
  function openManageIncentiveModal(s) {
    ...
    function handleSave() {
      ...
      if (idx >= 0) {
        ...
        const rateChanges = [];
        if (showService && updated.serviceIncentiveRate !== newServiceRate) rateChanges.push(`${t("incentiveType.service")} ${typeof updated.serviceIncentiveRate === "number" ? updated.serviceIncentiveRate : "-"}% → ${newServiceRate}%`);
        if (showSales && updated.salesIncentiveRate !== newSalesRate) rateChanges.push(`${t("incentiveType.sales")} ${typeof updated.salesIncentiveRate === "number" ? updated.salesIncentiveRate : "-"}% → ${newSalesRate}%`);
        ...
        if (rateChanges.length) {
          logActivity({
            module: "incentive", action: "incentive_configuration_changed",
            entityType: "staff", entityId: s.id,
            details: `${s.name} / ${rateChanges.join(", ")}`
          });
        }
      }
  ...
  function openStaffModal(staffMember) {
    ...
    document.getElementById("saveStaffBtn").addEventListener("click", () => {
      ...
      const staffList = load(KEY_STAFF, SEED_STAFF);
      let savedId = isEdit ? staffMember.id : "";
      if (isEdit) {
      ...
      save(KEY_STAFF, staffList);
      logActivity({
        module: "staff", action: isEdit ? "staff_edited" : "staff_created",
        entityType: "staff", entityId: savedId,
        details: `${name}${role ? " – " + role : ""}`
      });
      closeModal();
      showToast(t("toast.saved"), "success");
      renderStaffPage();
    ...
    if (isOwner) {
      table.querySelectorAll(".svc-price-input").forEach((input) => {
        input.addEventListener("change", () => {
          ...
          const row = svc[parseInt(input.dataset.idx, 10)];
          ...
          if (newPrice !== oldPrice) {
            logActivity({
              module: "services", action: "service_price_updated",
              entityType: "service", entityId: row.id || row.name,
              details: `${t("deal.colService")}: ${row.name} / ${t("th.price")}: ${fmt(oldPrice)} → ${fmt(newPrice)}`
            });
          }
          showToast(t("toast.saved"), "success");
        });
      ...
      table.querySelectorAll('[data-action="remove-service"]').forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          const removed = svc[idx];
          svc.splice(idx, 1);
          save(KEY_SERVICES, svc);
          if (removed) {
            logActivity({
              module: "services", action: "service_removed",
              entityType: "service", entityId: removed.id || removed.name,
              details: `${t("deal.colService")}: ${removed.name}`
            });
          }
          renderServicesPage();
        });
  ...
  function openAddServiceModal() {
    ...
    document.getElementById("newSvcSave").addEventListener("click", () => {
      ...
      services.push({ id, name, category, price });
      save(KEY_SERVICES, services);
      logActivity({
        module: "services", action: "service_added",
        entityType: "service", entityId: id,
        details: `${name} (${category}) / ${t("th.price")}: ${fmt(price)}`
      });
      closeModal();
      renderServicesPage();
    });
  ...
  function computeReportExportData() {
    ...
    const customerRows = customersForExport.map((c) => {
      ...
      const serviceTxsFiltered = custTxsFiltered.filter(isServiceTx);
      const totalVisits = serviceTxsFiltered.length;
      const firstVisitDate = serviceTxsFiltered.reduce((min, tx) => (!min || tx.date < min ? tx.date : min), "");
      ...
      const svcCount = {};
      serviceTxsFiltered.forEach((tx) => tx.services.forEach((r) => { if (r.service) svcCount[r.service] = (svcCount[r.service] || 0) + (r.qty || 1); }));
      const mostUsedEntry = Object.entries(svcCount).sort((a, b) => b[1] - a[1])[0];
      const mostUsedService = mostUsedEntry ? mostUsedEntry[0] : "—";
      // Only existing packageHistory fields (amount/paymentMethod/date/type/salesStaff) are used -
      // no invented data. Summarizes every purchase/top-up on record for this customer.
      const vipPackageInfo = (c.packageHistory || []).slice().sort((a, b) => a.date.localeCompare(b.date))
        .map((p) => `${p.type === "topup" ? "Top-Up" : "Purchase"} ${fmt(p.amount)} (${p.date}, ${p.paymentMethod})`).join("; ") || "—";
      return {
        name: c.name, phone: c.phone, type: c.type === "vip" ? "VIP" : "Normal", source: c.source || "—",
        firstVisitDate: firstVisitDate || "—", lastVisit: lastVisit || "—",
        totalVisits, revenueContribution, mostUsedService,
        vipPurchased, vipBalance: c.type === "vip" ? (c.vipBalance || 0) : null, vipPackageInfo
      };
  ...
  // Only fields that already exist safely in computeReportExportData()'s customerRows are offered
  // here - nothing invented. "currency" fields get the $ number format in Excel.
  const CUSTOMER_EXPORT_FIELDS = [
    { key: "name", header: "Customer Name", labelKey: "export.field.name", default: true },
    { key: "phone", header: "Phone Number", labelKey: "export.field.phone", default: true },
    { key: "type", header: "Customer Type", labelKey: "export.field.type", default: true },
    { key: "source", header: "Customer Source", labelKey: "export.field.source", default: false },
    { key: "firstVisitDate", header: "First Visit Date", labelKey: "export.field.firstVisit", default: false },
    { key: "lastVisit", header: "Last Visit Date", labelKey: "export.field.lastVisit", default: false },
    { key: "totalVisits", header: "Total Visits", labelKey: "export.field.totalVisits", default: false },
    { key: "revenueContribution", header: "Total Spending / Revenue Contribution", labelKey: "export.field.revenue", default: false, currency: true },
    { key: "mostUsedService", header: "Most-used Service", labelKey: "export.field.mostUsed", default: false },
    { key: "vipBalance", header: "VIP Balance", labelKey: "export.field.vipBalance", default: false, currency: true },
    { key: "vipPackageInfo", header: "VIP Package Purchase / Top-Up Information", labelKey: "export.field.vipPackageInfo", default: false }
  ];

  function openExportCustomerFieldsModal() {
    document.getElementById("modalTitle").textContent = t("export.modalTitle");
    document.getElementById("modalBody").innerHTML = `
      <p class="confirm-note">${t("export.modalHint")}</p>
      <div class="export-field-list" id="exportFieldListWrap">
      ...
      <div class="modal-actions">
        <button type="button" class="btn-outline-mini" id="exportFieldsCancelBtn">${t("modal.cancel")}</button>
        <button type="button" class="btn-primary" id="exportFieldsGoBtn">${t("reports.exportExcel")}</button>
      </div>
    `;
    const box = document.getElementById("modalBox");
    if (box) box.classList.remove("modal-wide");
    openModal();
    document.getElementById("exportFieldsCancelBtn").addEventListener("click", closeModal);
    document.getElementById("exportFieldsGoBtn").addEventListener("click", () => {
      const wrap = document.getElementById("exportFieldListWrap");
      const checked = [...wrap.querySelectorAll('input[type="checkbox"]:checked')].map((cb) => cb.value);
      if (!checked.length) {
        showToast(t("export.needOneField"), "error");
        return;
      }
      closeModal();
      exportReportsExcel(checked).catch((e) => {
  ...
  async function exportReportsExcel(customerFieldKeys) {
    ...
    if (tab === "customers") {
      // Owner picks which fields to include (Export Customer Data modal) - only those columns are
      // built, in the same order as CUSTOMER_EXPORT_FIELDS, and the rows still respect whatever
      // Reports filters (date range/staff/service/payment/customer type/source) were active when
      // Export Excel was clicked, since data.customerRows already reflects computeReportExportData()'s
      // filtered dataset.
      const selectedKeys = (customerFieldKeys && customerFieldKeys.length ? customerFieldKeys : CUSTOMER_EXPORT_FIELDS.filter((f) => f.default).map((f) => f.key));
      const fieldsToExport = CUSTOMER_EXPORT_FIELDS.filter((f) => selectedKeys.includes(f.key));
      const sheet = wb.addWorksheet("Customers");
      sheet.columns = fieldsToExport.map((f) => ({ header: f.header, key: f.key, width: Math.max(16, f.header.length + 4) }));
      styleHeaderRow(sheet);
      data.customerRows.forEach((row) => {
        const rowData = {};
        fieldsToExport.forEach((f) => {
          let v = row[f.key];
          if (f.key === "vipBalance" && v === null) v = "—";
          rowData[f.key] = v;
        });
        const r = sheet.addRow(rowData);
        fieldsToExport.forEach((f) => {
          if (f.currency && typeof rowData[f.key] === "number") r.getCell(f.key).numFmt = CURRENCY_FMT;
        });
      });
      if (!data.customerRows.length) sheet.addRow({});
    }
  ...
  function setupReportExport() {
    document.getElementById("exportReportExcelBtn").addEventListener("click", () => {
      // Reports > Customers > Export Excel asks the Owner which customer fields to include first;
      // every other tab exports immediately as before.
      if (currentReportTab() === "customers") {
        openExportCustomerFieldsModal();
        return;
      }
