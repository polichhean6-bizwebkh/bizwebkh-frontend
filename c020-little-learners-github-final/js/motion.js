(() => {
  ...
  if (lightbox && gallery.length) {
    ...
    const km = () => window.littleLearnersLanguage === 'km';
    prev.className = 'lightbox-nav prev'; prev.setAttribute('aria-label', km() ? 'រូបភាពមុន' : 'Previous image'); prev.textContent = '‹';
    next.className = 'lightbox-nav next'; next.setAttribute('aria-label', km() ? 'រូបភាពបន្ទាប់' : 'Next image'); next.textContent = '›';
    ...
    lightbox.addEventListener('touchstart', e => { const t = e.changedTouches[0]; touchStartX = t.clientX; touchStartY = t.clientY; }, { passive: true });
    lightbox.addEventListener('touchend', e => {
      const t = e.changedTouches[0]; const dx = t.clientX - touchStartX; const dy = t.clientY - touchStartY;
      if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)) { dx < 0 ? swap(current + 1) : swap(current - 1); }
    }, { passive: true });
  }
