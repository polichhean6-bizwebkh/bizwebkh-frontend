...
const translations = {
  en: {
    ...
    "hero.price": "Starting from $168,xxx",
    ...
    "plans.penthouseRequestTitle": "Penthouse layout details are available upon request.",
    "plans.penthouseRequestText": "Please contact the sales team on Telegram for the latest layout, availability, and pricing.",
  ...
  kh: {
    ...
    "hero.price": "ចាប់ផ្តើមពី $168,xxx",
    ...
    "plans.penthouseRequestTitle": "ព័ត៌មានប្លង់ Penthouse អាចស្នើសុំបាន។",
    "plans.penthouseRequestText": "សូមទាក់ទងក្រុមលក់តាម Telegram សម្រាប់ប្លង់ថ្មីៗ ភាពទំនេរ និងតម្លៃ។",
  ...
  zh: {
    ...
    "plans.penthouseRequestTitle": "顶层住宅户型资料可按需提供。",
    "plans.penthouseRequestText": "请通过 Telegram 联系销售团队，获取最新户型、可售情况与价格。",
...
const openPlanModal = (category) => {
  ...
  if (plan.requestOnly) {
    ...
    request.className = "plan-request";
    const heading = document.createElement("h4");
    heading.textContent = dictionary["plans.penthouseRequestTitle"] || translations.en["plans.penthouseRequestTitle"];
    const text = document.createElement("p");
    text.textContent = dictionary["plans.penthouseRequestText"] || translations.en["plans.penthouseRequestText"];
    const link = document.createElement("a");
    link.className = "btn btn-primary";
    link.href = TELEGRAM_URL;
    link.target = "_blank";
    link.rel = "noopener";
    link.textContent = dictionary["buttons.requestPenthouse"] || translations.en["buttons.requestPenthouse"];
    request.append(heading, text, link);
    planModalGrid.appendChild(request);
  } else {
    plan.images.forEach((src, index) => {
