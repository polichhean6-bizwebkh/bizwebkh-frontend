...
Owner (2nd account, for testing multi-Owner Activity Log):
  username: owner2
  password: demo123
...
- Export Excel (.xlsx) from any Reports tab, respecting the active filters, with an Owner-only
  field-selection modal on the Customers tab (choose exactly which columns to export)
- New Deal no longer has a general "Sales Staff" field - staff attribution comes only from each
  service row's own Staff selector (source of truth for reporting/incentives)
- VIP Package Purchase (new customer) and Sell/Top-Up VIP Package both support MULTIPLE Sales
  Staff per sale, shown as removable chips ("Sokunthea x", "+ Select Staff"); each selected staff
  earns their own Owner-configured Sales Incentive independently (never shown in the modal), and
  one VIP sale always records exactly one revenue event/transaction regardless of staff count
- Owner Activity Log (Owner-only, hidden from Cashier) - a simple, append-only record of who did
  what and when: customers created/edited/type-changed, transactions created/completed/edited/
  cancelled, VIP package purchases/top-ups/balance redemptions, staff added/edited/status changed,
  incentive configuration changes, and service added/price-updated/removed. Filterable by date
  range, user, module and action; supports multiple Owner accounts, each shown by their own name
